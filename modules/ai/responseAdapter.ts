...
import { V08_BLOCKED_REAL_MESSAGE } from "@/modules/integrations";
  ...
  if (!ai.recommendedMission) {
    return {
      ...
      automationBlockedMessage: ai.automationProposals?.some((p) => p.blockedReason)
        ...
        : undefined,
      integrationProposals: ai.integrationProposals,
      integrationBlockedMessage: ai.integrationProposals?.some((p) => p.blockedReason)
